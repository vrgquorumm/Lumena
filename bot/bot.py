...
async def _send_custom_values(chat_id: int, key: str, fallback_html: str,
                              values: dict[str, object], **kwargs):
    """Отправляет кастомный текст с несколькими динамическими значениями."""
    ct = brand.get_custom_text(key)
    if ct:
...
async def cmd_marriages(msg: Message):
    chat_marriages = marriages.get(msg.chat.id, {})
...
# ═══════════════════════════════════════════════════════
FORTUNES = ["Сегодня тебя ждёт приятный сюрприз 🌟","Будь осторожен с новыми знакомствами","Звёзды говорят: действуй смелее!","Хороший день для нового начала","Кто-то думает о тебе прямо сейчас 💭","Не бойся перемен — они к лучшему ✨","Сегодня лучше молчать и наблюдать","Тебя ждёт маленькая победа 🏆","Удача сегодня на твоей стороне 🍀","Будь готов к неожиданным новостям","Твои усилия скоро принесут плоды 🌱"]
EIGHT_BALL = ["Да ✅","Нет ❌","Возможно 🤔","Скорее да 👍","Скорее нет 👎","Спроси позже ⏳","Точно да! 🎯","Даже не думай 🚫","50/50 ⚖️","Всё указывает на да 🌟","Мои источники говорят — нет 🔮"]
TAROT = [("Шут","Новые начинания, спонтанность"),("Маг","Сила воли, мастерство"),("Жрица","Интуиция, тайны"),("Императрица","Изобилие, творчество"),("Emperor","Власть, стабильность"),("Влюблённые","Выбор, гармония"),("Колесница","Победа, контроль"),("Сила","Мужество, терпение"),("Отшельник","Мудрость, одиночество"),("Колесо Фортуны","Перемены, удача")]
...
async def cmd_choose(msg: Message, command: CommandObject = None):
    ...
    await msg.reply(f"🎯 Я выбираю: **{random.choice(opts)}**")
...
async def cmd_whois(msg: Message):
    ...
    title_str = profile_data.get("title", "")
...
async def cb_ank_media_done(cb: CallbackQuery):
    ...
    if not session or session.get("step") != _ank.PHOTO_STEP_IDX:
        return await cb.answer(
            brand.plain_text(
                "anketa_not_mine_callback",
                "Анкета не найдена или уже отправлена",
            ),
            show_alert=True,
        )
    ...
    await cb.answer(
        brand.plain_text(
            "media_sent_callback", "✅ Отправлено на модерацию!"
        )
    )
...
async def cb_ank_media_skip(cb: CallbackQuery):
    ...
    await _ank._finish_anketa(bot, uid, session)
    await cb.answer(
        brand.plain_text("media_skipped_callback", "⏭ Пропущено")
    )
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text=brand.plain_text("main_chat_btn", "💬 Чат проекта"),
                url="https://t.me/+_K2SJRYIhq9hYjFi",
            ),
            InlineKeyboardButton(
                text=brand.plain_text("main_channel_btn", "📢 Канал"),
                url="https://t.me/lmnfff",
            ),
        ],
        [InlineKeyboardButton(
            text=brand.plain_text("main_help_btn", "📖 Все команды"),
...
async def cb_verify_go(cb: CallbackQuery):
    if is_verified(cb.from_user.id):
        await cb.answer(
            brand.plain_text("verify_already", "Ты уже верифицирован ✅"),
            show_alert=False,
        )
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_edittext(cb: CallbackQuery):
    ...
    placeholders = brand.TEXT_PLACEHOLDERS.get(key)
    placeholder_note = (
        f"\nДоступные переменные: <code>{html.escape(placeholders)}</code>\n"
        "Оставь их в тексте — бот подставит значение автоматически."
...
async def cmd_revoke_anketa(msg: Message):
    ...
    try:
        await _send_custom(
            ...
            f"{brand.div()}",
            name=data.get("full_name") or data.get("username") or str(target_uid),
        )
    except Exception:
        pass
