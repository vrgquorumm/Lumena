...
async def _send_custom(bot_obj, chat_id: int, key: str, fallback: str,
    ...
    if ct:
        text, ents_data = ct
        if values:
            text, ents_data = brand.substitute_values(text, ents_data, values)
...
async def _answer_custom_values(msg_obj, key: str, fallback: str,
                                values: dict[str, object],
                                parse_mode: str = "HTML", **kwargs) -> None:
    """Отправляет кастомный текст шага с динамическими переменными."""
    ct = brand.get_custom_text(key)
    if ct:
        text, ents_data = brand.substitute_values(ct[0], ct[1], values)
        ents = _build_ents(ents_data)
        await msg_obj.answer(text, entities=ents or None, **kwargs)
    else:
        await msg_obj.answer(fallback, parse_mode=parse_mode, **kwargs)
...
def _media_done_kb(uid: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text=brand.plain_text("anketa_media_done_btn", "✅ Готово"),
            callback_data=f"ank_media_done:{uid}",
        ),
        InlineKeyboardButton(
            text=brand.plain_text("anketa_media_skip_btn", "⏭ Без медіа"),
            callback_data=f"ank_media_skip:{uid}",
        ),
    ]])
...
def _lang_kb() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text=brand.plain_text("anketa_lang_uk", "🇺🇦 Українська"),
            callback_data="ank_lang:uk",
        ),
        InlineKeyboardButton(
            text=brand.plain_text("anketa_lang_ru", "🇷🇺 Русский"),
            callback_data="ank_lang:ru",
        ),
    ]])
...
def reaction_kb(owner_uid: int) -> InlineKeyboardMarkup:
    ...
    rows = [[
        InlineKeyboardButton(
            text=brand.plain_text(
                "anketa_reaction_like_btn", f"❤️ {h}" if h else "❤️"
            ),
                             callback_data=f"ank_r:h:{owner_uid}"),
        InlineKeyboardButton(
            text=brand.plain_text(
                "anketa_reaction_dislike_btn", f"👎🏻 {d}" if d else "👎🏻"
            ),
    ...
    if chat_url:
        link_row.append(InlineKeyboardButton(
            text=brand.plain_text("anketa_reaction_chat_btn", "⭐ НАШ ЧАТ"),
            url=chat_url,
        ))
    link_row.append(InlineKeyboardButton(
        text=brand.plain_text(
            "anketa_reaction_create_btn", "📝 СТВОРИТИ АНКЕТУ"
        ),
...
def _make_mod_kb(app_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text=brand.plain_text("anketa_mod_accept_btn", "✅ Прийняти"),
                callback_data=f"ank_ok:{app_id}",
            ),
            InlineKeyboardButton(
                text=brand.plain_text("anketa_mod_reject_btn", "❌ Відхилити"),
                callback_data=f"ank_no:{app_id}",
            ),
        ],
        [
            InlineKeyboardButton(
                text=brand.plain_text(
                    "anketa_mod_comment_btn", "✏️ Правки (написать автору)"
                ),
                callback_data=f"ank_cm:{app_id}",
            ),
        ],
    ])
...
def make_my_anketa_kb(uid: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text=brand.plain_text("anketa_my_delete_btn", "🗑 Удалить анкету"),
            callback_data=f"ank_del:{uid}",
        ),
        InlineKeyboardButton(
            text=brand.plain_text("anketa_my_edit_btn", "✏️ Редактировать"),
            callback_data=f"ank_edit:{uid}",
        ),
    ]])
...
async def handle_media_step(bot_obj, msg) -> bool:
    ...
    if count >= 10:
        ...
        key = "anketa_media_limit_ru" if lang == "ru" else "anketa_media_limit_uk"
        fallback = (
        ...
        await _answer_custom_values(msg, key, fallback, {"count": count})
        await _finish_anketa(bot_obj, uid, session)
        return True

    emoji = "📷" if media_type == "photo" else "🎬"
    if lang == "ru":
        key = "anketa_media_accepted_ru"
        ack = (
            f"{emoji} *Медиа {count}/10 принято!*\n\n"
            "Отправь ещё фото/видео или нажми *«✅ Готово»*.\n"
            "_Чтобы пропустить медиа — «⏭ Без медіа»._"
        )
    else:
        key = "anketa_media_accepted_uk"
        ack = (
            f"{emoji} *Медіа {count}/10 прийнято!*\n\n"
            "Надішли ще фото/відео або натисни *«✅ Готово»*.\n"
            "_Щоб пропустити — «⏭ Без медіа»._"
        )
    ct = brand.get_custom_text(key)
    if ct:
        text, ents = brand.substitute_values(ct[0], ct[1], {"count": count})
        await msg.answer(text, entities=_build_ents(ents), reply_markup=_media_done_kb(uid))
    else:
...
async def handle_anketa_step(bot_obj, msg) -> bool:
    ...
    if step == PHOTO_STEP_IDX:
        ...
        if lo in _SKIP_MEDIA:
            ...
            if lang == "ru":
                key = "anketa_media_hint_ru"
                hint = (
                    f"📸 Отправь фото или видео. Добавлено: <b>{count}/10</b>.\n"
                    "Нажми <b>«✅ Готово»</b> чтобы завершить, "
                    "или <b>«⏭ Без медіа»</b> чтобы пропустить."
                )
            else:
                key = "anketa_media_hint_uk"
                hint = (
                    f"📸 Надішли фото або відео. Додано: <b>{count}/10</b>.\n"
                    "Натисни <b>«✅ Готово»</b> щоб завершити, "
                    "або <b>«⏭ Без медіа»</b> щоб пропустити."
                )
            await _answer_custom_values(
                msg, key, hint, {"count": count}, parse_mode="HTML"
            )
        return True
    ...
    if next_step >= len(qs):
        ...
        await _answer_custom_values(
            msg, "step_accepted", fallback,
            {"step": next_step, "total": total,
             "question": _lang_photo_text(lang)},
        )
    else:
        ...
        await _answer_custom_values(
            msg, "step_accepted", fallback,
            {"step": next_step, "total": total,
             "question": _md_to_html(question_text)},
        )
    return True
